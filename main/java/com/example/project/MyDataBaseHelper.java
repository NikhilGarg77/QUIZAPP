package com.example.project;import android.annotation.SuppressLint;
    ...
    public List<Question> getAllQuestionsList() {
        ...
        if (c.moveToFirst()) {
            do {
                ...
                @SuppressLint("Range") String questText= c.getString(c.getColumnIndex(QUESTION));
                question.setQuestion(questText);

                @SuppressLint("Range") String choice1Text= c.getString(c.getColumnIndex(CHOICE1));
                question.setChoice(0,choice1Text);

                @SuppressLint("Range") String choice2Text= c.getString(c.getColumnIndex(CHOICE2));
                question.setChoice(1,choice2Text);

                @SuppressLint("Range") String choice3Text= c.getString(c.getColumnIndex(CHOICE3));
                question.setChoice(2,choice3Text);

                @SuppressLint("Range") String choice4Text= c.getString(c.getColumnIndex(CHOICE4));
                question.setChoice(3,choice4Text);

                @SuppressLint("Range") String answerText= c.getString(c.getColumnIndex(ANSWER));
